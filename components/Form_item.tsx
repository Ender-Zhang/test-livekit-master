 ...
 * @LastEditTime: 2023-04-06 21:35:39
...
export function Form_item({ navigation, status }: { navigation: any, status: string }) {
  ...
  // for meeting
    ...
    myData.data = myData.data.filter((item: { type: string; status: string; }) => item.type == "Meeting" && item.status == "uncompleted");
  }
    ...
    // for practice
      ...
      myData.data = _.sortBy(myData.data, 'date');
      // myData.data = myData.data.filter((item: { type: string; }) => item.type == "uncompleted");
      myData.data = myData.data.filter((item: { type: string; status: string;}) => item.type == "Practice" && item.status == "uncompleted");
    }
