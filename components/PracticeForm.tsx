 ...
 * @LastEditTime: 2023-04-17 23:33:22
...
import Pressible from './Pressible_Pracitce';
...
//   const [timer, setTimer] = useState(0);
  ...
  return <View>
    ...
    {myData1.data.map((item, index) => (
      <View key={index}>
        <Pressible practiceSet={practiceSet} task_id={item.title} task_date={item.date} task_description={item.description} task_status={item.status} type={item.type} navigation={navigation} destination="Config" uncompleted={uncompleted} task_name={item.name}/>
      </View>
    ))}
  </View>
