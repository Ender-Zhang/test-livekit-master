 ...
 * @LastEditTime: 2023-07-28 09:08:08
...
export default function LoadingPage({ route, navigation } : any) {
    ...
    // get user info
      ...
      fetch("http://192.168.1.102:8080/interaction/dev/api/patient-cases/" + userId, requestOptions)
        .then(response => response.json())
        .then((result) => { 
          // console.log("loading result",result);
          writeUserData3(result);
        })
        .catch(error => console.log('errorUserInfo', error));
    };
    ...
    const handoleGetUserProtocol = () => {
      ...
      // console.log("userId",userId);
      // fetch("http://10.0.2.2:8080/interaction/dev/api/sssprotocols/" + userId, requestOptions)
      fetch("http://192.168.1.102:8080/interaction/dev/api/sssprotocols/" + userId, requestOptions)
        .then(response => response.json())
        .then((result) => { 
          console.log("loading result Protocol",result);
          writeUserData4(result);
        })
        .catch(error => console.log('errorProtocol', error));
    };
    ...
    const handoleGetLivekitToken = () => {
      ...
      // fetch("http://10.0.2.2:8080/interaction/dev/api/generate-token", requestOptions)
      fetch("http://192.168.1.102:8080/interaction/dev/api/generate-token", requestOptions)
